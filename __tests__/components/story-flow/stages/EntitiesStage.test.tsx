...
type PartialState = Partial<StoryFlowState> & Pick<StoryFlowState, "entities" | "loading">;
...
describe("EntitiesStage", () => {
  ...
  describe("entity details", () => {
    it("displays entity name in card", () => {
      const entities: EntityAsset[] = [
        { name: "Hero", description: "Main character", status: "completed", imageUrl: "http://hero.jpg" },
      ];
      const state = createMockState({ entities });
      render(<EntitiesStage state={state} actions={createMockActions()} />);
    ...
    it("displays entity description when available", () => {
      const entities: EntityAsset[] = [
        { name: "Hero", description: "The brave protagonist", status: "completed", imageUrl: "http://hero.jpg" },
      ];
      const state = createMockState({ entities });
      render(<EntitiesStage state={state} actions={createMockActions()} />);
    ...
    it("displays segment numbers when available", () => {
      const entities: EntityAsset[] = [
        { name: "Hero", description: "Main", segment: [1, 3, 5], status: "completed", imageUrl: "http://hero.jpg" },
      ];
      const state = createMockState({ entities });
      render(<EntitiesStage state={state} actions={createMockActions()} />);
    ...
    it("hides segment numbers when not available", () => {
      const entities: EntityAsset[] = [
        { name: "Hero", description: "Main", status: "completed", imageUrl: "http://hero.jpg" },
      ];
      const state = createMockState({ entities });
      render(<EntitiesStage state={state} actions={createMockActions()} />);
    ...
    it("hides segment numbers when empty array", () => {
      const entities: EntityAsset[] = [
        { name: "Hero", description: "Main", segment: [], status: "completed", imageUrl: "http://hero.jpg" },
      ];
      const state = createMockState({ entities });
      render(<EntitiesStage state={state} actions={createMockActions()} />);
      expect(screen.queryByText(/segments/i)).not.toBeInTheDocument();
  ...
  describe("regenerate action", () => {
    it("shows regenerate button when entity has imageUrl", () => {
      const entities: EntityAsset[] = [
        { name: "Hero", description: "Main", imageUrl: "http://hero.jpg", status: "completed" },
      ];
      const state = createMockState({ entities });
      render(<EntitiesStage state={state} actions={createMockActions()} />);
    ...
    it("disables button when status is generating", () => {
      const entities: EntityAsset[] = [
        { name: "Hero", description: "Main", imageUrl: "http://hero.jpg", status: "generating" },
      ];
      const state = createMockState({ entities });
      render(<EntitiesStage state={state} actions={createMockActions()} />);
    ...
    it("calls generateSingleEntity with correct index", () => {
      ...
      const entities: EntityAsset[] = [
        { name: "Hero", description: "Main", imageUrl: "http://hero.jpg", status: "completed" },
        { name: "Villain", description: "Bad guy", imageUrl: "http://villain.jpg", status: "completed" },
      ];
      const state = createMockState({ entities });
      const actions = { ...createMockActions(), generateSingleEntity: generateSingleEntityMock };
