...
type PartialState = Partial<StoryFlowState> & Pick<StoryFlowState, "segments" | "imageStatuses">;
...
describe("ImagesStage", () => {
  describe("counter display", () => {
    it("shows correct count of images with path", () => {
      const segments: Segment[] = [
        { text: "First", imagePrompt: "Prompt 1", imagePath: "http://img1.jpg" },
        { text: "Second", imagePrompt: "Prompt 2" },
        { text: "Third", imagePrompt: "Prompt 3", imagePath: "http://img3.jpg" },
      ];
  ...
  describe("image status rendering", () => {
    it("shows image when imagePath exists and not generating", () => {
      const segments: Segment[] = [
        { text: "Scene", imagePrompt: "A landscape", imagePath: "http://image.jpg" },
      ];
      const state = createMockState({ segments, imageStatuses: new Map() });
      render(<ImagesStage state={state} actions={createMockActions()} />);
    ...
    it("shows skeleton when status is generating", () => {
      ...
      render(<ImagesStage state={state} actions={createMockActions()} />);
      expect(document.querySelector("[class*='skeleton']") || document.querySelector("[class*='animate']")).toBeTruthy();
    });
  ...
  describe("retry action", () => {
    it("calls generateSingleImage with correct index on error retry", () => {
      ...
      imageStatuses.set(0, "error");
      const state = createMockState({ segments, imageStatuses });
      const actions = { ...createMockActions(), generateSingleImage: generateSingleImageMock };
  ...
  describe("prompt display", () => {
    it("displays imagePrompt truncated", () => {
      const longPrompt = "This is a very long prompt that should be truncated because it exceeds the line clamp limit";
      const segments: Segment[] = [
        { text: "Scene", imagePrompt: longPrompt, imagePath: "http://image.jpg" },
      ];
      const state = createMockState({ segments, imageStatuses: new Map() });
      render(<ImagesStage state={state} actions={createMockActions()} />);
