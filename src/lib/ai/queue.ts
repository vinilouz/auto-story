...
function calcBackoff(
  attempt: number,
  kind: BatchResult<any>["errorKind"],
): number {
...
export async function executeBatch<A extends ActionType>(
  ...
  for (const cfg of ACTIONS[action]) {
    ...
    if (!provider || !handler || !creds) {
      log.warn(
        `Skipping ${cfg.provider}/${cfg.model}: ${!provider ? "not registered" : !handler ? "no handler" : "no credentials"}`,
      );
      continue;
    }
    slots.push({
  ...
  if (slots.length === 0)
    throw new Error(`No available providers for ${action}`);

  const providerNames = [...new Set(slots.map((s) => s.providerName))];
  const info = providerNames
    .map(
      (n) =>
        `${n}: ${PROVIDER_RPM[n] ?? "∞"}rpm / ${PROVIDER_CONCURRENCY[n] ?? 3} concurrent`,
    )
    .join(", ");
  ...
  function requeue(
    ...
    backlog.push(job);
    log.warn(
      `  ↳ #${job.id + 1} backoff ${delay}ms — outro provider pode pegar antes`,
    );
  }
  ...
  async function worker(slot: ProviderSlot): Promise<void> {
    while (true) {
      ...
      if (
        ...
        log.info(
          `#${job.id + 1} ↪ crossover ${job.lastProvider} → ${slot.providerName}/${slot.model}`,
        );
      }
      ...
      try {
        ...
        const result: BatchResult<ActionMap[A]["res"]> = {
          ...
          status: "success",
          data,
          provider: `${slot.providerName}/${slot.model}`,
          durationMs: ms,
        };
        results[job.id] = result;
        completed++;
        log.success(
          `#${job.id + 1} ✓ ${slot.providerName}/${slot.model} (${ms}ms) [${completed}/${total}]`,
        );
        ...
        if (kind === "payload") {
          ...
          completed++;
          log.error(
            `#${job.id + 1} ✗ ${slot.providerName}/${slot.model} ${errorLabel(kind)} — sem retry [${completed}/${total}]\n  → ${msg.substring(0, 300)}`,
          );
          opts.onProgress?.(completed, total);
          opts.onResult?.(result);
          continue;
        ...
        if (job.attempts >= maxRetries) {
          ...
          completed++;
          log.error(
            `#${job.id + 1} ✗ ${slot.providerName}/${slot.model} ${errorLabel(kind)} — esgotou ${maxRetries} tentativas [${completed}/${total}]\n  → ${msg.substring(0, 300)}`,
          );
          opts.onProgress?.(completed, total);
          opts.onResult?.(result);
        } else {
          log.warn(
            `#${job.id + 1} ✗ ${slot.providerName}/${slot.model} ${errorLabel(kind)} (${ms}ms) — retry ${job.attempts}/${maxRetries}\n  → ${msg.substring(0, 300)}`,
          );
          requeue(job, kind);
        }
      }
  ...
  for (const slot of slots) {
    const concurrency = Math.min(
      PROVIDER_CONCURRENCY[slot.providerName] ?? 3,
      total,
    );
    log.info(
      `${slot.providerName}/${slot.model}: ${concurrency} workers (rate-limit: ${PROVIDER_RPM[slot.providerName] ?? "∞"}rpm)`,
    );
  ...
  const failSummary = Object.entries(byKind)
    .map(([k, n]) => `${n}×${k}`)
    .join(", ");
  log.info(
    `Batch concluído: ${ok}/${total} ok${failSummary ? ` | falhas: ${failSummary}` : ""}`,
  );
