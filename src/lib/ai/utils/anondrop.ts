 ...
 */
export async function uploadBufferToAnonDrop(buffer: Buffer, mimeType: string, filename: string): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(`AnonDrop upload failed: HTTP ${res.status} — ${responseText.substring(0, 200)}`)
  }
  const url = extractAnonDropUrl(responseText, filename)
  if (!url) {
    throw new Error(`AnonDrop: não foi possível extrair URL da resposta: "${responseText.substring(0, 200)}"`)
  }
...
export async function remoteUploadToAnonDrop(sourceUrl: string, filename?: string): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(`AnonDrop remote upload failed: HTTP ${res.status} — ${responseText.substring(0, 200)}`)
  }

  const url = extractAnonDropUrl(responseText, name)
  if (!url) {
    throw new Error(`AnonDrop: não foi possível extrair URL: "${responseText.substring(0, 200)}"`)
  }
