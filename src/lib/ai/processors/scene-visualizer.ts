...
function parseDescriptions(raw: string, expectedCount: number): Array<{ id: string; imagePrompt: string }> {
  let clean = raw.trim()
  ...
  if (arr.length !== expectedCount) {
    log.warn(`AI returned ${arr.length}/${expectedCount} descriptions (raw: ${raw.length} chars)`)
  }
  return arr
}
...
export async function generateSceneDescriptions(data: SceneVisualizationRequest) {
  ...
  log.info(`${total} segments → ${batchCount} batch(es), ${entities.length} entities`)
  ...
  for (let b = 0; b < batchCount; b++) {
    ...
    const batchInput = batchSegs.map((s, i) => ({ id: String(start + i + 1), scriptText: s.text.trim() }))
    ...
    while (true) {
      try {
        ...
        if (descriptions.length < batchSegs.length && retries < 2) {
          retries++
          log.warn(`Batch ${b + 1}: got ${descriptions.length}/${batchSegs.length} — retry ${retries}/2`)
          continue
        }

        break
      } catch (e: any) {
        if (retries < 2) { retries++; log.warn(`Batch ${b + 1} failed — retry ${retries}/2`); continue }
        throw e
      }
    }
    ...
    for (let i = 0; i < batchSegs.length; i++) {
      ...
      const imagePrompt = typeof desc?.imagePrompt === 'string' ? desc.imagePrompt
    ...
    log.success(`Batch ${b + 1}/${batchCount}: ${descriptions.length}/${batchSegs.length} ok`)
  }
