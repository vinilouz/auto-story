...
export async function POST(request: NextRequest) {
  try {
    ...
    log.info(`Audio request: ${text.length} chars, voice=${voice || 'default'}, batches=${targetBatchIndices || 'all'}`)

    const result = await generateAudio({
      text, voice, systemPrompt, targetBatchIndices,
      projectId: projectId,
      projectName: projectName,
    })

    const completed = result.batches.filter(b => b.status === 'completed').length
    const failed = result.batches.filter(b => b.status === 'error').length
    log.success(`Audio done: ${completed}/${result.batches.length} ok, ${failed} failed`)
