...
import { generateAndSaveVideoClipBatch, BatchClipRequest } from "@/lib/ai/processors/video-clip-generator"
...
export async function POST(request: NextRequest) {
  ...
  if (!clips?.length) {
    return new Response(JSON.stringify({ error: "Missing clips" }), { status: 400, headers: { 'Content-Type': 'application/json' } })
  }
