...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!commentatorDescription || !segments?.length) {
      return NextResponse.json({ error: "Missing commentatorDescription or segments" }, { status: 400 })
    }

    log.info(`Commentator request: ${segments.length} segments`)
    const result = await generateCommentsWithCommentator({ commentatorDescription, segments })
