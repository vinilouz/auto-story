...
import { Trash2, BookOpen, MessageSquare, Video } from "lucide-react"

type FlowType = 'simple' | 'commentator' | 'video-story'

interface Project {
  id: string; name: string; flowType: string
  createdAt: string; updatedAt: string; commentator?: any
}
...
export default function Home() {
  ...
  const loadProjects = async () => {
    try {
      ...
      if (res.ok) setProjects(
        (await res.json()).sort((a: Project, b: Project) =>
  ...
  const flowIcon = (type: string) => {
    if (type === 'video-story') return <Video className="w-4 h-4 text-purple-500" />
    if (type === 'with-commentator') return <MessageSquare className="w-4 h-4 text-green-500" />
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          >
            <CardHeader>
              ...
              <CardDescription>AI video clips (Grok/Veo) from audio</CardDescription>
            </CardHeader>
            <CardContent><Button className="w-full" size="lg">Start</Button></CardContent>
          </Card>
        </div>
